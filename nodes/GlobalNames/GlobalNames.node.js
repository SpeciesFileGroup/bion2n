...
class GlobalNames {
					...
					options: [
						{
							...
							required: false,
							description:
								"This flag changes how the position of a detected name in text is calculated. Normally a name's start and end positions are given as the number of UTF-8 characters from the beginning of the text. If bytesOffset flag is true, the start and end offsets are recalculated in the number of bytes.",
						},
						{
							...
							required: false,
							description:
								'The language of the text (e.g., eng). Language value is used for calculation of Bayesian odds. If this parameter is not given, english language is used by default. Currently only English and German languages are supported.',
						},
						{
						...
						},
						{
							...
							required: false,
							description:
								'Contains the text which will be checked for scientific names. If this parameter is not empty, the url paramter is ignored.',
						},
						{
							...
							required: false,
							description:
								'If this flag is true, the output returns a list of unique names, instead of a list of all name occurrences. Unique list of names does not provide position information of a name in the text.',
						},
						{
							displayName: 'URL',
							name: 'url',
						...
						},
						{
							...
							required: false,
							description:
								'When this flag is true, there is an addional verification step for detected names. This step requires internet connection and uses the Global Names verification API for queries.',
						},
						{
							...
							required: false,
							description:
								'Allows to see the context surrounding a name-string. The wordsAround parameter sets the number of words returned to output, which are located immediately before or after a detected name. Default is 0, maximum value is 5.',
						},
					],
				},
				{
	...
	async execute() {
		...
		for (let i = 0; i < items.length; i++) {
			...
			if (resource === 'parse') {
				const additionalFields = this.getNodeParameter('additionalFields', i);
				qs['with_details'] = this.getNodeParameter('withDetails', i);
...
//# sourceMappingURL=GlobalNames.node.js.map
